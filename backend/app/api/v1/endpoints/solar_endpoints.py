...
_solar_model_cache: Optional[SolarForecaster] = None

def get_solar_model() -> SolarForecaster:
    ...
    if os.path.exists(model_path):
        _solar_model_cache = SolarForecaster.load(model_path)
        return _solar_model_cache
    else:
        # If not trained yet, train on-the-fly
        _solar_model_cache = train_and_register_solar_model(days=90)
        return _solar_model_cache

@router.post("/train", summary="Train and register Solar XGBoost model")
def train_solar_model(
    days: int = Query(90, ge=30, le=365, description="Number of historical days to synthesize for training")
):
    """
    Train a new production XGBoost Solar Forecaster, evaluate on 20% test split,
    persist artifact to disk, and update the database model registry.
    """
    global _solar_model_cache
    forecaster = train_and_register_solar_model(days=days)
    _solar_model_cache = forecaster
...
@router.get("/metrics", summary="Get active Solar model metrics and feature importance")
def get_solar_metrics(db: Session = Depends(get_db)):
    """Retrieve performance scores and explainable feature importances for the active solar model."""
    run = db.query(ModelRun).filter(
        ModelRun.model_type == "solar",
    ...
    if not run:
        raise HTTPException(
            ...
            detail="No active solar model found in registry"
        )
...
@router.post("/predict/{plant_id}", summary="Predict hourly solar generation with uncertainty intervals")
def predict_solar_generation(
    ...
    """
    Execute inference with active Solar XGBoost model for a given plant:
    Applies feature transformations, predicts expected MW, P10 lower bound, P90 upper bound,
    and confidence scores. Enforces physical night zeroing.
    """
    ...
    if metadata["plant_type"] not in ("solar", "hybrid"):
        raise HTTPException(
    ...
    X_solar, _ = FeatureEngineer.create_feature_matrix(df_weather, metadata)

    # Run inference
    forecaster = get_solar_model()
    cap = float(metadata["capacity_mw"]) * (0.6 if metadata["plant_type"] == "hybrid" else 1.0)
    intervals_df = forecaster.predict_with_intervals(X_solar, capacity_mw=cap)
