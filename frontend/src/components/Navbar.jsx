          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2 pl-1 sm:pl-2 border-l border-slate-200">
                <div className="flex items-center gap-2 bg-emerald-50/80 border border-emerald-200/80 px-2.5 py-1 rounded-xl">
                  <div className="w-6 h-6 rounded-lg bg-emerald-600 text-white flex items-center justify-center font-bold text-xs">
                    {user?.full_name ? user.full_name.charAt(0).toUpperCase() : 'U'}
                  </div>
                  <div className="hidden xl:block text-left text-[11px] leading-tight">
                    <div className="font-bold text-slate-900 truncate max-w-[120px]">{user?.full_name || 'Stakeholder'}</div>
                    <div className="text-slate-500 capitalize">{user?.role?.replace('-', ' ') || 'Operator'}</div>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-1.5 pl-1 sm:pl-2 border-l border-slate-200">
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-semibold text-slate-700 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors flex items-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-xs transition-all flex items-center gap-1.5"
                >
            ...
            <div 
              ...
              className="cursor-pointer transition-transform active:scale-95 hidden sm:block"
            >
              <Badge 
